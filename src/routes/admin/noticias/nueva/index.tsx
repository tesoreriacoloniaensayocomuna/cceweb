...
export default component$(() => {
    ...
    const slugSignal      = useSignal('');
    const isAutoSlug      = useSignal(true);
    const imagePreview    = useSignal<string>('');
    const galleryPreviews = useSignal<string[]>([]);
    ...
    useVisibleTask$(({ track }) => {
        track(() => titleSignal.value);
        if (isAutoSlug.value) generateSlug(titleSignal.value).then(s => slugSignal.value = s);
    });
    ...
    const handleSubmit = $(async (e: SubmitEvent) => {
        ...
        try {
            const res  = await fetch('/api/admin/noticias/create', { method: 'POST', body: formData });
            const data = await res.json();

            if (res.ok && data.success) {
                successMsg.value = '¡Noticia publicada con éxito!';
                form.reset();
                titleSignal.value     = '';
                slugSignal.value      = '';
                imagePreview.value    = '';
    ...
    return (
        <div class="max-w-4xl mx-auto font-['Public_Sans'] pb-20">
            <div class="mb-8">
                ...
                <h1 class="text-3xl font-bold text-gray-900">Nueva Noticia</h1>
            </div>
            ...
            <form onSubmit$={handleSubmit} enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 space-y-4">
                            <div>
                                ...
                                <input type="text" name="title" required value={titleSignal.value}
                                    ...
                                    class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:border-primary outline-none text-lg font-bold" />
                            </div>
                            <div>
                                ...
                                <input type="text" name="slug" required value={slugSignal.value}
                                    onInput$={(e) => { isAutoSlug.value = false; slugSignal.value = (e.target as HTMLInputElement).value; }}
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-4 gap-3">
                                    {galleryPreviews.value.map((src, i) => (
                                    ...
                                    <label class="aspect-square rounded-lg border-2 border-dashed border-gray-300 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 hover:border-primary transition-colors gap-1">
                                        ...
                                        <input type="file" name="gallery" multiple accept="image/*" class="hidden"
                                            onChange$={(e) => {
                                                ...
                                                if (!files) return;
                                                galleryPreviews.value = [...galleryPreviews.value, ...Array.from(files).map(f => URL.createObjectURL(f))];
                                            }} />
                                    </label>
                                </div>
                            </div>
                    ...
                    <div class="space-y-5">
                        <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-200 space-y-4 text-sm">
                            <h2 class="text-xs font-bold uppercase tracking-wider text-gray-400 border-b pb-2">Publicación</h2>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-1 uppercase">Estado</label>
                                <select name="status" class="w-full p-2 border rounded-lg bg-white outline-none">
                            ...
                            <div class="aspect-video rounded-lg border-2 border-dashed border-gray-200 flex items-center justify-center relative overflow-hidden bg-gray-50 group cursor-pointer">
                                {imagePreview.value
                                    ...
                                    : <div class="flex flex-col items-center gap-1 text-gray-400">
                                        ...
                                        <span class="text-xs">Seleccionar portada</span>
                                      </div>
                                }
                                <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <span class="text-white text-xs font-bold bg-black/50 px-3 py-1 rounded-full">Elegir imagen</span>
                                </div>
                        ...
                        <button type="submit" disabled={isLoading.value}
                            ...
                            {isLoading.value
                                ...
                                : <><span class="material-symbols-outlined text-xl">send</span> Publicar Noticia</>
                            }
                        </button>
                    </div>
                </div>
